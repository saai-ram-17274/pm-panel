...
function build({ hours = 24 } = {}) {
  const s = spoc.summary();
  const feeds = feedCounts(hours);
  const last24 = spocLast24h(hours);
  ...
  const cards = `
    <div class="grid">
      <div class="card"><div class="muted">SPOC last 24h</div><div class="num">${last24.length}</div></div>
    </div>
    <div class="grid" style="margin-top:8px">
      <div class="card"><div class="muted">Competitive (24h)</div><div class="num">${feeds.product || 0}</div></div>
      <div class="card"><div class="muted">Analyst (24h)</div><div class="num">${feeds.analyst || 0}</div></div>
      <div class="card"><div class="muted">News (24h)</div><div class="num">${feeds.news || 0}</div></div>
    </div>`;
  ...
  const feedBlock = (label, items) => {
    if (!items.length) return `<p class="muted">No ${esc(label)} items in last ${hours}h.</p>`;
    return `<table><thead><tr><th>Source</th><th>Title</th></tr></thead><tbody>${
      items.map(i => `<tr>
        <td>${esc(i.product)}</td>
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><style>${css}</style></head><body>
    ...
    <h2>SPOC</h2>
    ${last24Table}
    ${breakdown('By priority', topPriority)}
    ${breakdown('By module', topModule)}
    ${recentSpoc}
    <h2>Feeds (last ${hours}h)</h2>
    <h3>Competitive</h3>
    ${feedBlock('competitive', sections.competitive)}
    <h3>Analyst</h3>
    ${feedBlock('analyst', sections.analyst)}
    <h3>Industry news</h3>
    ${feedBlock('news', sections.news)}
    <p class="muted" style="margin-top:24px">Sent automatically by pm-panel.</p>
  </body></html>`;

  const text =
    `Daily PM digest · ${today}\n` +
    `SPOC last ${hours}h: ${last24.length} tickets (total ${s.total || 0})\n` +
    `Feeds (24h): competitive=${feeds.product || 0} · analyst=${feeds.analyst || 0} · news=${feeds.news || 0}\n`;

  return {
    subject: `PM digest · ${today} · SPOC 24h ${last24.length} · Feeds ${(feeds.product||0)+(feeds.analyst||0)+(feeds.news||0)}`,
    html,
    text,
    stats: { spocLast24h: last24.length, spocTotal: s.total || 0, feeds, recentSpoc: recent.length },
  };
}

module.exports = { build, feedCounts, recentFeedItems, spocLast24h };
